...
const Signals = (() => {
  let current = "XAUUSD";
  let latest = null;     // { generatedAt, signals: [] } — signaux publiés (2 h)
  let history = [];      // tableau, plus récent en tête
  let liveCache = {};    // { SYMBOL: signal calculé en temps réel }
  ...
  function render() {
    ...
    // Priorité au signal calculé EN DIRECT ; sinon repli sur le dernier publié.
    const liveSig = liveCache[current];
    const stored  = (latest?.signals || []).filter(s => s.symbol === current);
    const shown   = liveSig ? [liveSig] : stored;
    grid.innerHTML = shown.length
      ? shown.map(card).join("")
      : `<div class="state"><div class="spinner"></div>Calcul du signal ${current} en direct…</div>`;
    ...
    if (stamp) {
      if (liveCache[current]) stamp.textContent = "🟢 En direct · " +
        new Date().toLocaleString("fr-FR");
      else if (latest) stamp.textContent = "Dernière mise à jour : " +
        new Date(latest.generatedAt).toLocaleString("fr-FR");
    }
  }

  /* ---------- Chargement des données -------------------------------------- */
  // Récupère l'historique + les signaux publiés (pour les notifications),
  // puis calcule le signal du symbole affiché EN DIRECT (prix temps réel).
  async function loadData(showSpinner) {
    const grid = document.getElementById("signals-grid");
    if (showSpinner && !liveCache[current])
      grid.innerHTML = `<div class="state"><div class="spinner"></div>Chargement des signaux…</div>`;
    try {
      ...
      if (l) { latest = l; history = h || []; checkNew(); }
    } catch (_) {}
    render();                 // affiche déjà l'historique + repli éventuel
    await computeLive(current);
    return true;
  }

  // Calcule un signal en temps réel pour un symbole via le moteur (Binance/PAXG).
  async function computeLive(sym) {
    if (typeof Engine === "undefined") return;
    try {
      Engine.CONFIG.interval = "1h";
      const s = await Engine.analyze(sym);
      s.timeLabel = s.time;
      liveCache[sym] = s;
      if (sym === current) render();
    } catch (e) {
      console.warn("[signals] calcul live impossible pour", sym, e.message);
      if (sym === current) render();   // on garde le repli sur le dernier publié
    }
  }
  ...
  function init() {
    document.querySelectorAll(".tabs button").forEach(btn =>
      btn.addEventListener("click", () => {
        ...
        render();
        computeLive(current);          // prix temps réel pour l'onglet choisi
      }));

    const refresh = document.getElementById("refresh");
    if (refresh) refresh.addEventListener("click", () => { liveCache = {}; loadData(true); });
